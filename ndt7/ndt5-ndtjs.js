 ...
 *
 * Source: https://github.com/m-lab/mlab-speedtest/blob/8e81856f0a9c210ad5ae55a2ce8c8a30ad7162a8/app/assets/js/ndt-browser-client.js
 */
...
function NDTjs(server, serverPort, serverProtocol, serverPath, callbacks,
  ...
  this.results = {
    c2sElapsed: undefined,
    c2sRate: undefined,
    c2sTotal: undefined,
    s2cElapsed: undefined,
    s2cRate: undefined,
    s2cTotal: undefined,
  };
  ...
  keepSendingData = function () {
    ...
    if (connectionOpen) {
      ...
      if (that.updateInterval && currentTime > (testStart + nextCallback)) {
        that.results.c2sTotal = totalSent - testConnection.bufferedAmount;
        that.results.c2sElapsed = currentTime - testStart;
  ...
  return function (messageType, messageContent) {
    ...
    if (state === 'WAIT_FOR_TEST_PREPARE' &&
      ...
      testConnection.onmessage = function (response) {
        ...
        currentTime = Date.now() / 1000.0;
        if (that.updateInterval && currentTime > (testStart + nextCallback)) {
          that.results.s2cTotal = receivedBytes;
          that.results.s2cElapsed = currentTime - testStart;
